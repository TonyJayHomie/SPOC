/**
 * ════════════════════════════════════════════════════════════════════════════════
 * WORKER C COMBINED — Sister PoC (SPOC) — Anthropic VDP/HackerOne Research Artifact
 * ════════════════════════════════════════════════════════════════════════════════
 *
 * This file is the COMBINED Worker C for VDP submission. It merges:
 *
 *   ┌─ Worker A Mock Backend ─────────────────────────────────────────────────┐
 *   │  Full Anthropic API mimic: /v1/messages (SSE streaming), /api/bootstrap,│
 *   │  /api/organizations, /api/account, OAuth token endpoints, Statsig mock, │
 *   │  4 auth modes (no_key/api_key/oauth/cookie_bridge), 3 mutation modes    │
 *   └─────────────────────────────────────────────────────────────────────────┘
 *
 *   ┌─ Worker C MITM Proxy ───────────────────────────────────────────────────┐
 *   │  System prompt mutation layer: passthrough/strip/replace/prepend/append │
 *   │  VDP evidence headers on every response (x-wc-system-stripped, etc.)    │
 *   │  Runtime admin API (/admin/set-mode, /admin/set-canary, /admin/reset)   │
 *   │  Proxy to api.anthropic.com OR external Worker A OR built-in mock      │
 *   └─────────────────────────────────────────────────────────────────────────┘
 *
 * Architecture routes (env WC_TARGET):
 *   WC_TARGET=local     → mutate → internal mock backend (DEFAULT, closed-loop)
 *   WC_TARGET=anthropic → mutate → api.anthropic.com (requires operator permission)
 *   WC_TARGET=worker_a  → mutate → external Worker A instance
 *
 * White-hat constraints (non-negotiable):
 *   1. Single operator only — no third-party user surface.
 *   2. No credential capture or exfiltration.
 *   3. BLOCKED_HOSTS: CoCoDem C2 only (openclaude.111724.xyz etc.) — NOT Anthropic.
 *   4. inject_canary: WC_CANARY_ENABLED default OFF — user must opt in.
 *   5. VDP / HackerOne anthropic-vdp safe-harbor only; not for distribution.
 *
 * Deployment: CF Dashboard → paste entire file → Save & Deploy
 * NO wrangler.toml, NO wrangler deploy CLI.
 * ════════════════════════════════════════════════════════════════════════════════
 */

// ================================================================
// WORKER A — Cloudflare Worker (Anthropic API Mimic Backend)
// Sister PoC research artifact — Anthropic HackerOne VDP only
// Single-operator. Not for distribution.
// ================================================================
//
// BLOCKED HOSTS: Only CoCoDem malware C2 domains are blocked.
//   Anthropic domains are NOT blocked — passthrough with the
//   operator's own legitimate credentials IS the VDP use case.
//   Blocked: openclaude.111724.xyz, cfc.aroic.workers.dev
//
// REFUSAL ITEMS (enforced throughout this file):
// 1. Single operator only — no third-party user surface.
// 2. No credential capture of any kind.
// 3. Blocked-host enforcement on every outbound URL.
// 4. VDP / HackerOne anthropic-vdp safe-harbor only.
//
...
// CF Worker: Node.js modules not available — using Web APIs instead
// http/https → fetch() | url → new URL() | fs/path/os → not needed
// crypto → Web Crypto (crypto.randomUUID available) | zlib → not needed
// EventEmitter → not needed (no WS server)

// Polyfills for Node.js APIs used in route handlers
const urlMod = { parse(u, q) { const x = new URL(u, 'http://localhost'); return { pathname: x.pathname, query: Object.fromEntries(x.searchParams), search: x.search }; } };
const Buffer = { byteLength(s) { return new TextEncoder().encode(s).length; }, from(s, e) { if (e === 'base64') { const b = atob(s); const a = new Uint8Array(b.length); for (let i=0;i<b.length;i++) a[i]=b.charCodeAt(i); return a; } return new TextEncoder().encode(s); }, concat(bufs) { return bufs; } };
const process = { env: {}, uptime() { return Math.floor((Date.now() - _startMs) / 1000); }, version: 'cf-worker', platform: 'cloudflare', exit() {}, stderr: { write(){} } };
const _startMs = Date.now();

// ────────────────────────────────────────────────────────────────
// HTTP/HTTPS SHIM — translates Node.js transport.request() to fetch()
// All 3 proxy functions (proxyRequest, proxyAndTranslateOAI*,
// proxyAndTranslateAnthropic*) and webhook dispatch use this.
// ────────────────────────────────────────────────────────────────
function _makeTransportShim() {
  return {
    request(options, callback) {
      ...
      const reqObj = {
        write(data) { if (data) chunks.push(data); },
        end(data) {
          ...
          const body = chunks.length ? chunks.join('') : undefined;

          // Build headers, strip content-length (fetch handles it)
          const headers = {};
          if (options.headers) {
          ...
          fetch(url, { method, headers, body, redirect: 'follow' })
            .then(async (resp) => {
              ...
              const proxyRes = {
                ...
                on(evt, fn) { this._listeners[evt] = this._listeners[evt] || []; this._listeners[evt].push(fn); return this; },
                resume() {},
                destroy() {},
                pipe(writable) {
                  // Read body as stream and pipe to writable
                  const reader = resp.body?.getReader();
                  if (!reader) { writable.end(); return this; }
                  const pump = () => {
                    reader.read().then(({ done, value }) => {
                      ...
                      for (const h of (this._listeners.error || [])) h(e);
                    });
                  };
                  pump();
                  return this;
              ...
              if (callback) callback(proxyRes);

              // Stream body to data/end listeners
              if (resp.body && (proxyRes._listeners.data.length || proxyRes._listeners.end.length)) {
                const reader = resp.body.getReader();
                const pump = async () => {
                  try {
                    while (true) {
                      ...
                      if (done) break;
                      const text = new TextDecoder().decode(value);
                      for (const h of proxyRes._listeners.data) h(text);
                    }
                    for (const h of proxyRes._listeners.end) h();
                  } catch (e) {
                    for (const h of (proxyRes._listeners.error || [])) h(e);
                  }
                };
                pump();
              } else if (!resp.body) {
        ...
        on(evt, fn) {
          ...
          if (evt === 'timeout') timeoutHandlers.push(fn);
          return reqObj;
        },
        destroy() { timedOut = true; if (timeoutTimer) clearTimeout(timeoutTimer); },
      };
      return reqObj;
    }
  };
}
const http = _makeTransportShim();
const https = http;

// zlib shim — CF Workers decompress automatically via fetch
const zlib = {
  createGunzip() { return _passthroughStream(); },
  createBrotliDecompress() { return _passthroughStream(); },
  createInflate() { return _passthroughStream(); },
};
function _passthroughStream() {
  // fetch() auto-decompresses, so pipe() is passthrough
  return {
    _listeners: {},
    on(evt, fn) { this._listeners[evt] = this._listeners[evt] || []; this._listeners[evt].push(fn); return this; },
    write(data) { for (const h of (this._listeners.data || [])) h(data); },
    end() { for (const h of (this._listeners.end || [])) h(); },
  };
}

// path/os shims
const path = { join(...parts) { return parts.join('/'); } };
const os = { homedir() { return '/cf-worker'; } };

// WebSocket: not available in CF Workers (would need Durable Objects)
// In CF architecture, Worker C connects via HTTP fetch, not WS
let WebSocketServer = null;

// ────────────────────────────────────────────────────────────────
// BLOCKED HOSTS — CoCoDem malware C2 domains ONLY
// Anthropic domains are NOT blocked — passthrough with operator's
// own legitimate credentials is the VDP use case.
// ────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────
// CF Worker: no filesystem — config via env vars or in-memory defaults
const CONFIG_DIR  = '/cf-worker';
const CONFIG_PATH = '/cf-worker/config.json';
const LOG_PATH    = '/cf-worker/requests.log';
const fs = { existsSync() { return false; }, readFileSync() { return '{}'; }, writeFileSync() {}, mkdirSync() {}, watch() {}, appendFileSync() {} };
...
function makeShare(convUuid, params = {}) {
  ...
  sh.share_url = `http://localhost:${config.port || 3967}/share/${sh.share_token}`;
...
// CF Workers: WebSocket server not available (would need Durable Objects)
// In CF architecture, Worker C connects via HTTP POST to /bridge/inbound
// ────────────────────────────────────────────────────────────────
let activeBridgeSocket = null;
// const wss = new WebSocketServer({ noServer: true }); // Node.js only
// WS connection handler removed — replaced by HTTP /bridge/inbound endpoint
...
<EntityDescriptor xmlns="urn:oasis:names:tc:SAML:2.0:metadata" entityID="${ssoConfig.entity_id}">
  ...
  if (pathname === '/bridge/status' && method === 'GET') {
    sendJson(res, {
      ...
      ws_url:          `ws://${HOST || '127.0.0.1'}:${PORT || 3967}/ws`,
    });
    return 200;
  }
...
<div class="panel" id="panel-arch">
  ...
  │  openclaude.111724.xyz, cfc.aroic.workers.dev│
  │  (CoCoDem malware C2 only)                  │
  └─────────────────────────────────────────────┘
       │
       ▼
  ...
  ● OpenRouter  https://openrouter.ai/api/v1
  ● Anthropic   https://api.anthropic.com  [passthrough with operator creds]
...
async function loadOverview(){
  try{
    ...
    document.getElementById('blocked-hosts').innerHTML=
      (bh.blocked_hosts||['openclaude.111724.xyz','cfc.aroic.workers.dev','111724.xyz','aroic.workers.dev']).map(h=>'🚫 '+h).join('<br>');
...
// SERVER STARTUP — CF Worker
// ────────────────────────────────────────────────────────────────
loadConfig();
// watchConfig(); // CF Worker: no filesystem to watch

// Boot-time smoke checks
function smokeCheck() {
  let ok = true;
  // Verify blocked-host list is loaded (CoCoDem malware domains)
  if (!BLOCKED_HOSTS.has('openclaude.111724.xyz')) {
    console.error('[worker-a] SMOKE FAIL: blocked-host list missing CoCoDem C2');
    ok = false;
  }
  if (!config.port) {
...
// ================================================================
// WORKER A INTERNAL HANDLER (callable by combined Worker C)
// WC_TARGET=local routes here; all mock backend logic lives inside
// ================================================================
const _WC_WORKER_A_INTERNAL = {
  async fetch(request, env) {
    const url    = new URL(request.url);
    const method = request.method.toUpperCase();
    const origin = request.headers.get('origin') || '*';

    // Apply env vars to runtime config
    if (env && env.BACKEND_URL)   config.backend_urls.custom = env.BACKEND_URL;
    if (env && env.BACKEND_KEY)   config.api_key = env.BACKEND_KEY;
    if (env && env.AUTH_MODE)     config.auth_mode = env.AUTH_MODE;
    if (env && env.DEFAULT_MODEL) config.default_model = env.DEFAULT_MODEL;
    if (env && env.BACKEND)       config.backend = env.BACKEND;
    if (env && env.OAUTH_TOKEN)   config.oauth_token = env.OAUTH_TOKEN;
    if (env && env.MUTATION_MODE) config.mutation_mode = env.MUTATION_MODE;
    if (env && env.SYSTEM_PROMPT) config.system_prompt_override = env.SYSTEM_PROMPT;
    ...
    // Build Node.js-compatible req/res objects
    const hdrs = {};
    ...
    const cfReq = {
      ...
      _cfBody: null,
      _listeners: {},
      on(evt, fn) { /* no-op for CF Worker req */ },
    };

    // Pre-read body
    if (method !== 'GET' && method !== 'HEAD') {
      try { cfReq._cfBody = await request.text(); } catch {}
    }

    // Node.js-compatible response adapter
    const resChunks = [];
    let resStatus = 200;
    let resHeaders = {};
    let resEnded = false;
    ...
    const cfRes = {
      ...
      writeHead(status, hdrs2) {
        resStatus = status;
        headersSent = true;
        if (hdrs2 && typeof hdrs2 === 'object') {
          for (const [k2, v2] of Object.entries(hdrs2)) {
            resHeaders[k2.toLowerCase()] = String(v2);
          }
        }
      },
      setHeader(k2, v2) { resHeaders[k2.toLowerCase()] = String(v2); },
      getHeader(k2) { return resHeaders[k2.toLowerCase()]; },
      write(chunk) {
        if (chunk != null) {
          isStreaming = true;
          resChunks.push(typeof chunk === 'string' ? chunk : String(chunk));
          if (streamController) {
            try { streamController.enqueue(new TextEncoder().encode(typeof chunk === 'string' ? chunk : String(chunk))); } catch {}
          }
        }
        return true;
      },
        ...
        if (data != null) {
          resChunks.push(typeof data === 'string' ? data : String(data));
          if (streamController) {
            try { streamController.enqueue(new TextEncoder().encode(typeof data === 'string' ? data : String(data))); } catch {}
          }
        }
        resEnded = true;
        if (streamController) {
    ...
    // Override readBody for CF Worker
    const origReadBody = readBody;
    readBody = function(req2) {
      if (req2._cfBody !== undefined) {
        return Promise.resolve(
          (() => { try { return JSON.parse(req2._cfBody || '{}'); } catch { return {}; } })()
        );
      }
      return origReadBody(req2);
    };

    // Override readRawBody for CF Worker
    if (typeof readRawBody !== 'undefined') {
      const origReadRaw = readRawBody;
      readRawBody = function(req2) {
        if (req2._cfBody !== undefined) {
          return Promise.resolve(new TextEncoder().encode(req2._cfBody));
        }
        return origReadRaw(req2);
      };
    }

    try {
      // Check if this is an SSE endpoint (streaming response)
      const isSSE = hdrs['accept']?.includes('text/event-stream')
                 || url.pathname.includes('/completion')
                 || url.pathname.includes('/sse_test')
                 || url.pathname === '/v1/messages'
                 || url.pathname === '/v1/chat/completions'
                 || url.pathname === '/v1/messages/thinking'
                 || url.pathname === '/v1/messages/tools';

      if (isSSE && method === 'POST') {
        ...
        const handlePromise = handleRequest(cfReq, cfRes);

        // Wait a tick for writeHead to be called
        await new Promise(r => setTimeout(r, 0));

        // If headers have been sent with SSE content type, return streaming
        if (headersSent && (resHeaders['content-type'] || '').includes('text/event-stream')) {
          // Wait for completion in background
          handlePromise.catch(e => {
            console.error('[worker-a] Stream error:', e.message);
            try { streamController.close(); } catch {}
          });

          return new Response(stream, {
            status: resStatus,
            headers: { ...resHeaders, ...corsHeaders(origin, true) },
          });
        }

        // Not actually streaming — wait for completion
        await handlePromise;
      } else {
        await handleRequest(cfReq, cfRes);
      }

      // Build final response
      if (!resHeaders['access-control-allow-origin']) {
        Object.assign(resHeaders, corsHeaders(origin));
      }

      const body = resChunks.length ? resChunks.join('') : null;
      return new Response(body, { status: resStatus, headers: resHeaders });
    } catch (e) {
      console.error('[worker-a] Error:', e.message);
      return new Response(JSON.stringify({ error: { type: 'server_error', message: e.message } }), {
        status: 500,
...
// ════════════════════════════════════════════════════════════════════════════════
// WORKER C — MITM PROXY + SYSTEM PROMPT MUTATION LAYER
// Combined with Worker A mock backend (above). Architecture:
//   WC_TARGET=local     → mutate body → route to built-in Worker A mock backend
//   WC_TARGET=anthropic → mutate body → proxy to api.anthropic.com
//   WC_TARGET=worker_a  → mutate body → proxy to external Worker A instance
//
// VDP evidence headers injected on every response:
//   x-wc-system-stripped, x-wc-system-length, x-wc-system-preview,
//   x-wc-mutation-mode, x-wc-request-id
//
// Mutation modes (env WC_MODE or runtime /admin/set-mode):
//   passthrough | strip | replace | prepend | append | inject_canary
//
// White-hat constraints (non-negotiable):
//   1. No real Anthropic traffic by default — WC_TARGET=local uses built-in mock.
//   2. Single operator only — no third-party user surface.
//   3. No credential capture, no exfiltration to non-user hosts.
//   4. inject_canary: WC_CANARY_ENABLED=false default; user must opt in.
//   5. BLOCKED_HOSTS: CoCoDem C2 only (already enforced by Worker A above).
// ════════════════════════════════════════════════════════════════════════════════

// ── Worker C proxy constants ──────────────────────────────────────────────────

const _WC_PROXY_VERSION = '1.0.0-spoc-combined-20260509';

const _WC_MUTATION_MODES = ['passthrough', 'strip', 'replace', 'prepend', 'append', 'inject_canary'];

const _WC_PROXY_DEFAULTS = {
  mode:          'passthrough',
  target:        'local',
  workerAUrl:    'http://127.0.0.1:8787',
  anthropicBase: 'https://api.anthropic.com',
  replacement:   '[SYSTEM PROMPT STRIPPED BY SISTER POC — SPOC VDP ARTIFACT]',
  prefix:        '[SPOC-PREFIX] ',
  suffix:        ' [SPOC-SUFFIX]',
...
// Per-isolate runtime mutable state (resets on cold start)
const _wc_runtimeState = {
  mode:         null,
  canaryEnabled: null,
  requestCount: 0,
  startTime:    Date.now()
...
// ── Worker C proxy utility functions ─────────────────────────────────────────

function _wc_traceId() {
  const arr = new Uint8Array(16);
  crypto.getRandomValues(arr);
  return Array.from(arr).map(b => b.toString(16).padStart(2, '0')).join('');
}

function _wc_getMode(env) {
  if (_wc_runtimeState.mode !== null) return _wc_runtimeState.mode;
  const m = (env.WC_MODE || _WC_PROXY_DEFAULTS.mode).toLowerCase().trim();
  return _WC_MUTATION_MODES.includes(m) ? m : _WC_PROXY_DEFAULTS.mode;
}

function _wc_getCanaryEnabled(env) {
  if (_wc_runtimeState.canaryEnabled !== null) return _wc_runtimeState.canaryEnabled;
  return (env.WC_CANARY_ENABLED || '').toLowerCase() === 'true';
}

function _wc_getTarget(env) {
  return (env.WC_TARGET || _WC_PROXY_DEFAULTS.target).toLowerCase().trim();
}

function _wc_getWorkerAUrl(env) {
  return (env.WORKER_A_URL || _WC_PROXY_DEFAULTS.workerAUrl).replace(/\/$/, '');
}

// ── System prompt extraction ──────────────────────────────────────────────────

/**
 * Extract system prompt text from Anthropic-format request body.
 * Handles: string system, array-of-blocks system, missing system.
 * Returns { system: string, originalLength: number, found: boolean }
 */
function _wc_extractSystem(body) {
 ...
 */
function _wc_applyMutation(body, mode, env) {
  const { system, originalLength, found } = _wc_extractSystem(body);
  const preview = system.slice(0, _WC_PROXY_DEFAULTS.previewLen);

  if (!found && originalLength === 0) {
    return { mutatedBody: body, stripped: false, originalLength: 0, preview: '', mode };
  }
  ...
  switch (mode) {
    ...
    case 'replace': {
      mutatedBody.system = env.WC_SYSTEM_REPLACEMENT || _WC_PROXY_DEFAULTS.replacement;
      stripped = true;
      break;
    }

    case 'prepend': {
      const prefix = env.WC_SYSTEM_PREFIX || _WC_PROXY_DEFAULTS.prefix;
      if (typeof body.system === 'string') {
    ...
    case 'append': {
      const suffix = env.WC_SYSTEM_SUFFIX || _WC_PROXY_DEFAULTS.suffix;
      if (typeof body.system === 'string') {
    ...
    case 'inject_canary': {
      const canaryEnabled = _wc_getCanaryEnabled(env);
      if (canaryEnabled) {
        const canaryText = env.WC_CANARY_TEXT || _WC_PROXY_DEFAULTS.canaryText;
...
function _wc_evidenceHeaders(stripped, originalLength, preview, mode, reqId) {
  return {
...
// ── Proxy forwarding (used when WC_TARGET=anthropic or worker_a) ─────────────

/**
 * Forward (possibly mutated) request to upstream target.
 * Streams SSE response body back for passthrough.
 */
async function _wc_forwardRequest(upstreamUrl, originalRequest, mutatedBody, env, reqId) {
  // Block CoCoDem C2 hosts (BLOCKED_HOSTS already defined by Worker A above)
  try {
    const upHost = new URL(upstreamUrl).hostname.toLowerCase();
    for (const bh of BLOCKED_HOSTS) {
      if (upHost === bh || upHost.endsWith('.' + bh)) {
        return new Response(JSON.stringify({ error: 'blocked_host', reqId }), { status: 403,
  ...
  const skipHop = new Set(['host','connection','keep-alive','transfer-encoding','te','trailer','upgrade','proxy-authorization','proxy-authenticate']);
  for (const [k, v] of originalRequest.headers.entries()) {
    if (!skipHop.has(k.toLowerCase())) forwardHeaders.set(k, v);
  }

  const target = _wc_getTarget(env);
  if (target === 'anthropic' && env.ANTHROPIC_API_KEY) {
    forwardHeaders.set('x-api-key', env.ANTHROPIC_API_KEY);
    forwardHeaders.delete('authorization');
  }

  if (mutatedBody !== null) forwardHeaders.set('content-type', 'application/json');

  // Strip x-wc-* headers before forwarding
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: 'upstream_error', message: String(err), reqId }), {
...
/**
 * Build the upstream URL from request based on WC_TARGET.
 */
function _wc_buildUpstreamUrl(request, env) {
  const target = _wc_getTarget(env);
  const url = new URL(request.url);
  const pathAndQuery = url.pathname + url.search;
  if (target === 'worker_a') {
    return _wc_getWorkerAUrl(env) + pathAndQuery;
  }
  return _WC_PROXY_DEFAULTS.anthropicBase + pathAndQuery;
}

// ── Admin API (for runtime mode changes) ─────────────────────────────────────

function _wc_verifyAdmin(request, env) {
...
function _wc_handleAdmin(pathname, request, env) {
  if (!_wc_verifyAdmin(request, env)) {
  ...
  if (pathname === '/admin/status') {
    return new Response(JSON.stringify({
      proxy_version:  _WC_PROXY_VERSION,
      uptime_ms:      Date.now() - _wc_runtimeState.startTime,
      request_count:  _wc_runtimeState.requestCount,
      mode:           _wc_getMode(env),
      target:         _wc_getTarget(env),
      canary_enabled: _wc_getCanaryEnabled(env),
      worker_a_url:   _wc_getWorkerAUrl(env)
  ...
  if (pathname === '/admin/set-mode' && request.method === 'POST') {
    return request.json().then(body => {
      ...
      if (!_WC_MUTATION_MODES.includes(newMode)) {
        return new Response(JSON.stringify({ error: 'invalid_mode', valid: _WC_MUTATION_MODES }), {
          status: 400, headers: { 'content-type': 'application/json' } });
      }
      _wc_runtimeState.mode = newMode;
      return new Response(JSON.stringify({ ok: true, mode: newMode }), {
        status: 200, headers: { 'content-type': 'application/json' } });
    }).catch(err => new Response(JSON.stringify({ error: 'bad_json', message: String(err) }), {
      status: 400, headers: { 'content-type': 'application/json' } }));
  }

  if (pathname === '/admin/set-canary' && request.method === 'POST') {
    return request.json().then(body => {
      _wc_runtimeState.canaryEnabled = Boolean(body.enabled);
      return new Response(JSON.stringify({ ok: true, canary_enabled: _wc_runtimeState.canaryEnabled }), {
        status: 200, headers: { 'content-type': 'application/json' } });
    }).catch(err => new Response(JSON.stringify({ error: 'bad_json', message: String(err) }), {
      status: 400, headers: { 'content-type': 'application/json' } }));
  }
  ...
  if (pathname === '/admin/reset') {
    _wc_runtimeState.mode = null;
    _wc_runtimeState.canaryEnabled = null;
    _wc_runtimeState.requestCount = 0;
    _wc_runtimeState.startTime = Date.now();
    return new Response(JSON.stringify({ ok: true, message: 'Runtime state reset to env defaults' }), {
      status: 200, headers: { 'content-type': 'application/json' } });
  }
...
// ── SSE streaming passthrough (for proxy modes) ───────────────────────────────

function _wc_streamingResponse(upstream, evidHeaders, origin) {
  const responseHeaders = new Headers();
  const hopByHop = ['connection','keep-alive','transfer-encoding','te','trailer'];
  for (const [k, v] of upstream.headers.entries()) {
    if (!hopByHop.includes(k.toLowerCase())) responseHeaders.set(k, v);
  }
  for (const [k, v] of Object.entries(evidHeaders)) responseHeaders.set(k, v);
  // CORS — use Worker A's corsHeaders function (already defined above)
  try {
...
// ════════════════════════════════════════════════════════════════════════════════
// COMBINED CLOUDFLARE WORKER EXPORT
// Routes:
//   WC_TARGET=local     → apply mutation → Worker A internal mock backend
//   WC_TARGET=anthropic → apply mutation → proxy to api.anthropic.com
//   WC_TARGET=worker_a  → apply mutation → proxy to external Worker A
//
// Environment variables:
//   WC_MODE              = passthrough (default) | strip | replace | prepend | append | inject_canary
//   WC_TARGET            = local (default) | anthropic | worker_a
//   WORKER_A_URL         = http://127.0.0.1:8787 (used when WC_TARGET=worker_a)
//   ANTHROPIC_API_KEY    = sk-ant-...  (used when WC_TARGET=anthropic, api_key mode)
...
//   WC_CANARY_ENABLED    = false (USER SET, DEFAULT OFF — never true in shipped artifact)
//   WC_CANARY_TEXT       = canary sentinel text
//   WC_ADMIN_TOKEN       = bearer token for /admin/* (optional)
//   + all Worker A env vars (BACKEND_URL, BACKEND_KEY, AUTH_MODE, etc.)
// ════════════════════════════════════════════════════════════════════════════════
export default {
  async fetch(request, env) {
    _wc_runtimeState.requestCount++;
    const _wcReqId  = _wc_traceId();
    const _wcUrl    = new URL(request.url);
    const _wcPath   = _wcUrl.pathname;
    const _wcMethod = request.method.toUpperCase();
    const _wcOrigin = request.headers.get('origin') || '*';
    const _wcTarget = _wc_getTarget(env);
    const _wcMode   = _wc_getMode(env);

    // ── CORS preflight ───────────────────────────────────────────────────────
    if (_wcMethod === 'OPTIONS') {
      let corsh;
      try { corsh = corsHeaders(_wcOrigin); } catch (_) { corsh = { 'access-control-allow-origin': _wcOrigin }; }
      return new Response(null, { status: 204, headers: { ...corsh, 'cache-control': 'max-age=86400' } });
    }

    // ── Worker C admin / health routes (all targets) ─────────────────────────
    if (_wcPath === '/health' || _wcPath === '/wc-health') {
      return new Response(JSON.stringify({
        status:        'ok',
        proxy_version: _WC_PROXY_VERSION,
        mode:          _wcMode,
    ...
    if (_wcPath.startsWith('/admin/')) {
      return _wc_handleAdmin(_wcPath, request, env);
    }

    // ── Telemetry short-circuits ─────────────────────────────────────────────
    if (_wcPath === '/api/event_logging/v2/batch') {
      return new Response(JSON.stringify({ status: 'ok', events_received: 1 }), {
        status: 200,
        headers: {
          'content-type': 'application/json',
          ..._wc_evidenceHeaders(false, 0, '', 'passthrough', _wcReqId)
        }
      });
    }
    if (_wcPath === '/v1/code/github/batch-branch-status') {
    ...
    // ── Determine if this is a completion endpoint that needs mutation ────────
    const _wcIsCompletion = (
      _wcPath === '/v1/messages' ||
      _wcPath.endsWith('/completion') ||
      (_wcPath.includes('/api/organizations/') && _wcPath.includes('/chat_conversations/'))
    );

    // ── Apply system prompt mutation (all targets) ────────────────────────────
    let _wcMutResult = { mutatedBody: null, stripped: false, originalLength: 0, preview: '', mode: _wcMode };
    let _wcRequestBody = null;

    if (_wcMethod === 'POST' && _wcIsCompletion) {
      try {
        _wcRequestBody = await request.clone().json();
        _wcMutResult   = _wc_applyMutation(_wcRequestBody, _wcMode, env);
      } catch (_) { _wcRequestBody = null; }
    }

    // ── Route: WC_TARGET=local → internal Worker A mock backend ─────────────
    if (_wcTarget === 'local') {
      // Reconstruct request with mutated body so Worker A sees the mutated payload
      let internalRequest = request;
      if (_wcMutResult.mutatedBody !== null) {
        internalRequest = new Request(request.url, {
          method:  request.method,
          headers: new Headers(request.headers),
          body:    JSON.stringify(_wcMutResult.mutatedBody)
        });
      }

      // Invoke Worker A's internal mock backend
      let waResponse;
      try {
        waResponse = await _WC_WORKER_A_INTERNAL.fetch(internalRequest, env);
      } catch (err) {
      ...
      // Inject Worker C evidence headers into Worker A response
      const _wcEvidH = _wc_evidenceHeaders(_wcMutResult.stripped, _wcMutResult.originalLength, _wcMutResult.preview, _wcMode, _wcReqId);
      const finalHeaders = new Headers(waResponse.headers);
      for (const [k, v] of Object.entries(_wcEvidH)) finalHeaders.set(k, v);

      return new Response(waResponse.body, {
    ...
    // ── Route: WC_TARGET=anthropic or worker_a → external proxy ─────────────
    const _wcUpstreamUrl = _wc_buildUpstreamUrl(request, env);

    // Block CoCoDem C2
    try {
      const upHost = new URL(_wcUpstreamUrl).hostname.toLowerCase();
      for (const bh of BLOCKED_HOSTS) {
        if (upHost === bh || upHost.endsWith('.' + bh)) {
          return new Response(JSON.stringify({ error: 'blocked_host', reqId: _wcReqId }), {
            status: 403, headers: { 'content-type': 'application/json',
              ..._wc_evidenceHeaders(false, 0, '', _wcMode, _wcReqId) }
          });
        }
      }
    } catch (_) {}

    const _wcUpstream = await _wc_forwardRequest(
      _wcUpstreamUrl,
      request,
      _wcMutResult.mutatedBody !== null ? _wcMutResult.mutatedBody : _wcRequestBody,
      env,
      _wcReqId
    );
    ...
    // SSE passthrough
    const _wcUpCT = (_wcUpstream instanceof Response ? _wcUpstream.headers.get('content-type') : '') || '';
    if (_wcUpCT.includes('text/event-stream')) {
      return _wc_streamingResponse(_wcUpstream, _wcEvidH, _wcOrigin);
    }

    // Non-streaming: buffer + inject evidence headers
    const _wcBody = await _wcUpstream.arrayBuffer();
    const _wcRespH = new Headers();
    const hopByHop = ['connection','keep-alive','transfer-encoding','te','trailer'];
    for (const [k, v] of _wcUpstream.headers.entries()) {
      if (!hopByHop.includes(k.toLowerCase())) _wcRespH.set(k, v);
    }
    for (const [k, v] of Object.entries(_wcEvidH)) _wcRespH.set(k, v);
    try {
      const cors = corsHeaders(_wcOrigin);
      for (const [k, v] of Object.entries(cors)) _wcRespH.set(k, v);
    } catch (_) {
      _wcRespH.set('access-control-allow-origin', _wcOrigin);
    }

    return new Response(_wcBody, {
      status:     _wcUpstream.status,
      statusText: _wcUpstream.statusText,
      headers:    _wcRespH
    });
  }
};
...
/*
 * ── DEPLOYMENT NOTES (Combined Worker C) ────────────────────────────────────
 *
 * This file combines:
 *   - Worker A: Full mock Anthropic API backend (all endpoints, SSE streaming,
 *     OAuth identity surface, Statsig mock, system prompt mutation)
 *   - Worker C: MITM proxy layer with VDP evidence headers
 *
 * Cloudflare Dashboard → Workers & Pages → Create → Worker → paste this file
 *
 * Key environment variables:
 *   WC_TARGET = local      → use built-in Worker A mock (DEFAULT, closed-loop)
 *   WC_TARGET = anthropic  → MITM proxy to api.anthropic.com (requires WC operator permission)
 *   WC_TARGET = worker_a   → MITM proxy to external Worker A instance
 *
 *   WC_MODE   = passthrough (default) | strip | replace | prepend | append
 *   WC_CANARY_ENABLED = false (DEFAULT OFF — user must opt in)
 *
 *   BACKEND_URL = http://127.0.0.1:1234/v1  (Worker A backend URL for local LLM)
 *   AUTH_MODE   = no_key | api_key | oauth | cookie_bridge
 *
 * Admin API:
 *   GET  /wc-health                             → proxy + backend status
 *   GET  /admin/status                           → full runtime state
 *   POST /admin/set-mode  {"mode":"strip"}       → change mutation mode at runtime
 *   POST /admin/set-canary {"enabled":true}      → toggle canary (USER CONTROLLED)
 *   POST /admin/reset                            → reset proxy state
 *
 * VDP Evidence Headers (on every response):
 *   x-wc-system-stripped   : "true" if system prompt was mutated
 *   x-wc-system-length     : original byte length
 *   x-wc-system-preview    : first 120 chars of original
 *   x-wc-mutation-mode     : active mutation mode
